...
using (var db = new MyDbContext())
{
    ...
    db.Questionnaires.AddRange([
        new Questionnaire { Sections = [new() { Questions = [new() { TranslatedQuestions = [new("en", @"Change of address?")] }] }], Answers = [ new() { Content = @"New York" }] },
        new Questionnaire { Sections = [new() { Questions = [new() { TranslatedQuestions = [new("de", @"Addressänderung?")] }] }], Answers = [ new() { Content = @"Zürich has one Umlaut" }] },
...
using (var db = new MyDbContext())
{
    ...
    {
        Console.WriteLine($"{++i}: {questionnaire.Sections.Single().Questions.Single().TranslatedQuestions.Single().Value}: {questionnaire.Answers.Single().Content}");
    }
    // prints:
